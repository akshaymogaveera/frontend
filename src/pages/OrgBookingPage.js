  ...
  return (
    ...
    >
      <CardActionArea
        ...
        sx={{ p: isMobile ? 2 : 2.5, height: '100%', alignItems: 'stretch', display: 'flex' }}
      >
        <Box
          sx={{
            position: 'absolute',
...
function WalkInDialog({ open, onClose, category, orgId, onSuccess, onError }) {
  ...
  const handleBook = async () => {
    ...
    try {
      ...
      if (userNote.trim() && data?.id) {
        apiCall(ENDPOINTS.APPOINTMENT_NOTES(data.id), {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
      ...
      let errorMsg = err.message || 'Booking failed.';
  ...
  useEffect(() => {
    ...
    const fetchPreview = async () => {
      ...
      try {
        const d = await apiCall(ENDPOINTS.APPOINTMENTS_UNSCHEDULED_COUNT(category.id));
        if (!mounted) return;
    ...
    >
      ...
      <DialogContent sx={{ pt: 1.5 }}>
        <Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>
          You're about to join the queue.
        </Typography>
            ...
            {category?.estimated_time > 0 &&
              (() => {
                ...
                return waitStr ? (
                  <Typography
                    variant="caption"
                    sx={{ display: 'block', mt: 0.5, fontWeight: 600, color: 'warning.main' }}
                  >
        ...
        <Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>
          You are joining the walk-in queue for{' '}
...
export default function OrgBookingPage() {
  ...
  const fetchLanding = useCallback(async () => {
    ...
    setError('');
    try {
      const data = await apiCall(ENDPOINTS.ORGANIZATION_LANDING(orgId));
          ...
          if (explicitlyOpen) {
            if (token) setSelectedCat(cat);
              ...
              openLogin({
                ...
                preSelectOrg: data.organization,
                preSelectCat: cat,
              });
          }
        } else {
  ...
  if (error)
    return (
      ...
      >
        <Alert severity="error" sx={{ mb: 2, borderRadius: 2, maxWidth: 400, width: '100%' }}>
          {error}
        </Alert>
  ...
  return (
    ...
    >
        ...
        <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', mb: 2 }}>
          {isLoggedIn ? (
            <Button
              ...
              sx={{ color: 'rgba(255,255,255,0.75)', textTransform: 'none', fontWeight: 500 }}
            >
              Home
            </Button>
          ) : (
          ...
          <Box sx={{ display: 'flex', justifyContent: 'center', mb: 1 }}>
            <Chip
              ...
              sx={{
                ...
                fontSize: 11,
                fontWeight: 600,
                bgcolor: (theme) => theme.palette.custom?.mint,
        ...
        {(() => {
          ...
          return (
            ...
            >
              ...
              <Typography
                variant="body2"
                sx={{ flex: 1, wordBreak: 'break-word', overflowWrap: 'anywhere' }}
              >
                {addrParts.join(', ')}
              </Typography>
              {url && (
                  ...
                  <Box
                    ...
                    sx={{
                      ...
                      '&:hover': {
                        bgcolor: '#c62828',
                        transform: 'scale(1.1)',
                        boxShadow: '0 4px 14px rgba(229,57,53,0.55)',
                      },
                    }}
                  >
      ...
      <BookingConfirmDialog
        ...
        onClose={() => {
          setCreatedApptOpen(false);
          setCreatedApptErrorOpen(false);
          setCreatedAppt(null);
          setCreatedApptError(null);
        }}
        ...
        onViewAppointment={() => {
          setCreatedApptOpen(false);
          setCreatedApptErrorOpen(false);
          navigate('/appointments', { state: { openApptId: createdAppt?.id } });
        }}
      />
    </Box>
  );
